...
test('public branch pages load maps only through view map links', function () {
    ...
    foreach ($pages as $page) {
        ...
        expect($source)
            ...
            ->and($source)->not->toContain('BranchLocatorMap')
            ->and($source)->toContain('href={branch.map_link}')
            ->and($source)->toContain('target="_blank"');
    }
});

test('the dedicated branch page uses service-style cards without changing the landing cards', function () {
    $root = dirname(__DIR__, 2).DIRECTORY_SEPARATOR;
    ...
    expect($branchPage)
        ->toContain(
            ...
            'group flex flex-col overflow-hidden bg-card shadow-lg',
            'branch.image_url ??',
